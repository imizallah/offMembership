...
const EVPDetails = ({ history,
    ...
    useEffect(() => {
        if (activities !== null) {
            ...
            updateUserDetails(
                activities.users.filter((el) => el._id === userId)[0]
            )
        }
        // eslint-disable-next-line
    }, [activities])
    const { fullName, phoneNumber, address, photo, credit } = userDetails;

    return (
        <>
            <div className='d-flex mb-3 align-items-center justify-content-between'>
                <h2 className='mb-0'>
                    <BsArrowLeft onClick={() => { history.goBack() }} style={{ cursor: 'pointer' }} /> User Details
                </h2>
...
const mapStateToProps = ({ analytics }) => {
    ...
    return { loading, error, message, activities };
};
export default (connect(mapStateToProps, { getAnalytics: getActivities }))(EVPDetails)
